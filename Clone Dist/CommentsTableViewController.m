        ...
        if (self.foundWord.length > 0) {
            ...
            [userQuery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                if (!error && objects.count > 0) {
                    searchResultsArray = [objects filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"SELF.username beginswith[c] %@", self.foundWord]].mutableCopy;
                } else {
                    [KVNProgress showErrorWithStatus:NSLocalizedString(@"Couldn't get search results", @"nope")];
                }
            }];
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:self.tableView]) {
        ...
        [userQuery whereKey:@"objectId" equalTo:[[comment objectForKey:@"user"] objectId]];
        [userQuery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
            if (!error) {
                PFUser *commentUser = objects.lastObject;
                cell.textLabel.text = commentUser.username;
            } else {
                [KVNProgress showErrorWithStatus:NSLocalizedString(@"Couldn't get user info", @"error")];
            }
        }];
    } else {
        PFUser *foundUser = searchResultsArray[indexPath.row];
...
{
    if ([tableView isEqual:self.autoCompletionView]) {
        [self acceptAutoCompletionWithString:[NSString stringWithFormat:@"@%@", self.foundWord]];
    }
}
