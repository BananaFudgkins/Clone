...
- (IBAction)changeprofilephotoPressed:(UIButton *)sender {
    ...
    UIAlertAction *choosePhoto = [UIAlertAction actionWithTitle:NSLocalizedString(@"Choose Existing", @"choose") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {
            [self performSelectorOnMainThread:@selector(changeprofilephotoPressed:) withObject:self waitUntilDone:NO];
        }
    }
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [imageFile saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if (!error) {
            ...
            [savingHUD hide:YES];
            [KVNProgress showErrorWithStatus:NSLocalizedString(@"Couldn't save profile photo", @"nope")];
        }
        if (succeeded) {
            [savingHUD hide:YES];
